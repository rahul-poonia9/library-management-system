...
export const useBookIssues = () => {
  ...
  const createMutation = useMutation({
    mutationFn: (bookIssue: Omit<BookIssue, 'id' | 'created_at' | 'updated_at'>) =>
  ...
  const returnMutation = useMutation({
    mutationFn: (id: number) => bookIssueService.returnBook(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['bookIssues'] });
    },
  });
...
export const useBookIssuesByStudent = (studentId: number) => {
...
export const useBookIssuesByBook = (bookId: number) => {
  const { data: bookIssues = [], ...rest } = useQuery({
